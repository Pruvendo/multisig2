...
(* TODO *)
Context `{LocalStateField' uint32}.
#[private, pure]
...
Ursus Definition submitUpdate (codeHash :  optional  ( uint256 )) (owners :  optional  ( listArray uint256 )) (reqConfirms :  optional  ( uint8 )) (lifetime :  optional  ( uint64 )): UExpression ( uint64) true .
   ...
   ::// if ( #{codeHash}->hasValue() ) then { {_:UExpression _ true} } .
   ...
   :://m_updateRequests := m_updateRequests->set(!{updateId}, {} (*UpdateRequest(updateId, !{index}, #{0}, #{0}, !{sender}, #{codeHash}->reset(), #{owners}, #{reqConfirms}, #{lifetime})*));_| .
   :://_confirmUpdate(!{updateId}, !{index})  .
   ::// return_ !{updateId} |.
Defined. 

Context `{LocalStateField' (listArray CustodianInfoLRecord)}.
#[public, view]
Ursus Definition getCustodians : UExpression (listArray CustodianInfoLRecord) false .
   ...
   :://return_ !{custodians} |.
Defined. 
Context `{LocalStateField' (listArray TransactionLRecord)}.
#[public, view]
...
Ursus Definition _confirmTransaction (txn : TransactionLRecord) (custodianIndex :  uint8): UExpression PhantomType false .
   ...
   :://if ( {}(* TODO поправить сравнение в либе ((#{txn}->Transaction_ι_signsReceived + #{1}) >= #{txn}->Transaction_ι_signsRequired)*) ) then { {_:UExpression _ false} } else { {_:UExpression _ false} }  .
...
Ursus Definition _getSendFlags (value :  uint128) (allBalance :  boolean): UExpression ( uint8 ** uint128) false .
   ...
   ::// new 'flags : (  uint8 ) @ "flags"  := {} (* TODO поправить операции (FLAG_IGNORE_ERRORS | FLAG_PAY_FWD_FEE_FROM_BALANCE) *) ;_|.
   ::// if ( #{allBalance} ) then { {_:UExpression _ false} } .
   ::// {flags} := {} (* TODO поправить операции (FLAG_IGNORE_ERRORS | FLAG_SEND_ALL_REMAINING) *) .
   ::// {value_ } := uint128(#{0})  |.
